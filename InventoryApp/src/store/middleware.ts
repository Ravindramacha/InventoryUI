...
startAppListening({
  ...
  effect: async (action, listenerApi) => {
    ...
    if (action.type.includes('create') || action.type.includes('update') || action.type.includes('delete')) {
      const operationType = action.type.includes('create') ? 'created' : 
                           action.type.includes('update') ? 'updated' : 'deleted';
  ...
  effect: async (action) => {
    ...
    if (action.type.includes('fetch') || action.type.includes('api')) {
      ...
      if (performanceMetrics.slowActions.length > 50) {
        performanceMetrics.slowActions = performanceMetrics.slowActions.slice(-50);
      }
    }
  },
});
