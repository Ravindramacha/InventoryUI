...
import {
  ...
  useGetUomsByDimensionId,
  useLanguages,
  usePostProductMasterForm,
  useProductTypes,
  usePutProductMasterForm,
  useSalesStatus,
  useUomDimension,
} from '../../api/ApiQueries';
...
const ApplicationForm: React.FC<ApplicationFormPageProps> = ({
  ...
  const [uomRows, setUomRows] = useState<UomData[]>(
    initialData?.productMasterUomDto &&
      initialData.productMasterUomDto.length > 0
      ? initialData.productMasterUomDto
      : initialUOMRows
  );
  const [formData, setFormData] = useState<PostProductMasterForm>(
    initialData || initialProductMasterForm
  );
  ...
  const [numberFields, setNumberFields] =
    useState<Attribute[]>(initialNumberFields);
  const [dropDownFields, setDropDownFields] = useState<Attribute[]>(
  ...
  const { data: uomsByDimension = [] } = useGetUomsByDimensionId(
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (mode === 'edit' && productMasterId) {
      ...
      // --- Edit Mode ---
      updateMutate(
        { id: productMasterId, data: finalFormData },
        {
          onSuccess: () => {
        ...
        <Grid container spacing={2}>
          ...
          <Grid size={{ xs: 12, sm: 12, md: 6, lg: 4 }}>
            <Autocomplete
              ...
              value={
                productTypes.find(
                  (p) => p.productTypeId === formData.productTypeId
                ) ?? null
              }
          ...
          <Grid size={{ xs: 12, sm: 12, md: 6, lg: 4 }}>
            <Autocomplete
              ...
              value={
                productGroups.find(
                  (p) => p.productGroupId === formData.productGroupId
                ) || null
              }
              getOptionLabel={(option) =>
            ...
            <Autocomplete
              ...
              value={
                productCategories.find(
                  (p) => p.productCategoryId === formData.productCategoryId
                ) || null
              }
              getOptionLabel={(option) =>
          ...
          <Grid size={{ xs: 12, sm: 12, md: 6, lg: 4 }}>
            <Autocomplete
              ...
              value={
                uomDimensions.find(
                  (p) => p.uomDimId === formData.unitOfMeasurement
                ) || null
              }
              getOptionLabel={(option) =>
          ...
          <Grid container spacing={2}>
            ...
            <Grid size={{ xs: 12, sm: 12, md: 6, lg: 4 }}>
              <Autocomplete
                ...
                value={
                  salesStatuses.find(
                    (p) => p.salesStatusId === formData.manufacturerId
                  ) || null
                }
                getOptionLabel={(option) =>
                  `${option.salesStatusDesc} (${option.salesStatusCode})` || ''
